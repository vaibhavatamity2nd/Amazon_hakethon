...
def extract_text(image_path):
    result = reader.readtext(image_path)  # Use EasyOCR to read the text
    extracted_text = " ".join([item[1] for item in result])  # Combine all text from OCR results
    return extracted_text

# Define the function for extracting value and unit from text
def extract_value_and_unit_ocr(extracted_text, entity_name):
    # Extract the numeric value (using regex to match numbers)
    value_match = re.search(r"(\d+(\.\d+)?)", extracted_text)

    if value_match:
        value = float(value_match.group(0))  # Extract numeric value
    else:
        return None, None  # No valid value found

    # Find the correct unit for the entity_name, including short forms
    allowed_units = entity_unit_map.get(entity_name, [])
    for unit in allowed_units:
        if unit in extracted_text:
            return value, unit

    return value, None  # Return value if found, even if unit is missing

# Parse the entity_value into numeric value and unit, handle ranges and errors
def parse_entity_value(entity_value):
    try:
        # Handle ranges like 'kilogram to 15 kilogram'
        if ' to ' in entity_value:
            entity_value = entity_value.split(' to ')[0]  # Take the first part

        value, unit = entity_value.split(maxsplit=1)  # Split the numeric value from the unit
        value = float(value)
        return value, unit
    except Exception as e:
        print(f"Error parsing entity_value: {entity_value} - {e}")
        return None, None
